...
# Setup script for local development environment (optional separate realm working copy)

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$SCRIPT_DIR"
REALM_DIR="${REALM_DIR:-$PROJECT_DIR/../realm}"
...
# Copy data files (example configs only; secrets stay in .env / local data/)
if [ -d "$PROJECT_DIR/data" ]; then
  mkdir -p "$REALM_DIR/data"
  cp "$PROJECT_DIR/data/"*.example.json "$REALM_DIR/data/" 2>/dev/null || true
fi
