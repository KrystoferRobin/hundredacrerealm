...
- **Keep your webhook URL private**: Never commit it to version control (use `.env` or the admin panel, which saves to `data/discord_webhook_settings.json` — that file is gitignored)
- **If a webhook was ever exposed**: Delete it in Discord Server Settings → Integrations → Webhooks and create a new one; old URLs remain valid until revoked
