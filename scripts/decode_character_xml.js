...
function parseAttributeBlock(block) {
  ...
  if (block.attribute) {
    if (Array.isArray(block.attribute)) {
      ...
      Object.keys(block.attribute).forEach(key => {
        if (key !== '_') {
          result[key] = block.attribute[key];
        }
      });
    }
  }
  ...
  if (block.attributeList) {
    if (Array.isArray(block.attributeList)) {
      block.attributeList.forEach(list => {
        result[list.keyName] = {};
        if (list.attributeVal) {
          if (Array.isArray(list.attributeVal)) {
            list.attributeVal.forEach(val => {
              Object.keys(val).forEach(key => {
                if (key !== '_') {
                  result[list.keyName][key] = val[key];
                }
              });
            });
          } else {
            Object.keys(list.attributeVal).forEach(key => {
              if (key !== '_') {
                result[list.keyName][key] = list.attributeVal[key];
              }
            });
          }
        }
      ...
      result[block.attributeList.keyName] = {};
      if (block.attributeList.attributeVal) {
        if (Array.isArray(block.attributeList.attributeVal)) {
          block.attributeList.attributeVal.forEach(val => {
            Object.keys(val).forEach(key => {
              if (key !== '_') {
                result[block.attributeList.keyName][key] = val[key];
              }
            });
          });
        } else {
          Object.keys(block.attributeList.attributeVal).forEach(key => {
            if (key !== '_') {
              result[block.attributeList.keyName][key] = block.attributeList.attributeVal[key];
            }
          });
        }
      }
...
function main() {
  ...
  const outputPath = 'parsed_sessions/learning-woodsgirl/decoded_character_data.json';
  fs.writeFileSync(outputPath, JSON.stringify(result, null, 2));
  
  // Save formatted timeline
  const timelinePath = 'parsed_sessions/learning-woodsgirl/character_timeline.json';
  ...
  // Save daily data
  const dailyPath = 'parsed_sessions/learning-woodsgirl/character_daily_data.json';
  ...
  console.log('\n=== SAMPLE TIMELINE ===');
  const sampleDates = Object.keys(formattedTimeline).slice(0, 5);
