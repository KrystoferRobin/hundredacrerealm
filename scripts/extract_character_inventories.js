...
function extractCharacterInventories(sessionId) {
  const xmlPath = path.join(process.cwd(), 'parsed_sessions', sessionId, 'extracted_game.xml');
  const sessionPath = path.join(process.cwd(), 'parsed_sessions', sessionId, 'parsed_session.json');
  const outputPath = path.join(process.cwd(), 'parsed_sessions', sessionId, 'character_inventories.json');
