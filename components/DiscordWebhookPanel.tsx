...
function DestinationSettingsSection({
  title,
  description,
  webhookLabel,
  webhookPlaceholder,
  ...
  return (
    <div className="rounded-lg border border-gray-200 p-5 bg-gray-50">
      ...
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">{webhookLabel}</label>
          <input
            type="text"
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange({ ...settings, botName: e.target.value })}
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
          />
        </div>
        ...
        <div className="flex items-center">
          ...
          />
          <label className="ml-2 block text-sm text-gray-700">{enableLabel}</label>
        </div>
...
export default function DiscordWebhookPanel({ className = '' }: DiscordWebhookPanelProps) {
  const [discordSettings, setDiscordSettings] = useState<WebhookSettings>(defaultSettings);
  const [havenSettings, setHavenSettings] = useState<WebhookSettings>(defaultSettings);
  ...
  const announcementsEnabled =
    (discordSettings.enabled && Boolean(discordSettings.webhookUrl)) ||
    (havenSettings.enabled && Boolean(havenSettings.webhookUrl));
  ...
  const loadData = async () => {
    try {
      ...
      if (data.discord || data.settings) {
        setDiscordSettings(data.discord || data.settings);
      }
      if (data.haven) {
        setHavenSettings(data.haven);
      }
      if (data.sessions) {
  ...
  const saveDiscordSettings = async () => {
    ...
    try {
      const response = await fetch('/api/admin/discord-webhook', {
        ...
        body: JSON.stringify({
          action: 'update-discord-settings',
          ...discordSettings,
        }),
      });

      const data = await response.json();
      setMessage(data.success ? 'Discord settings saved successfully!' : 'Error saving Discord settings');
    } catch (error) {
  ...
  const saveHavenSettings = async () => {
    ...
    try {
      const response = await fetch('/api/admin/discord-webhook', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          action: 'update-haven-settings',
          ...havenSettings,
        }),
      });

      const data = await response.json();
      setMessage(data.success ? 'Haven settings saved successfully!' : 'Error saving Haven settings');
    } catch (error) {
      console.error('Error saving Haven settings:', error);
      setMessage('Error saving Haven settings');
    } finally {
      setLoading(false);
    }
  };

  const sendNotification = async (type: string, sessionId?: string, template?: string) => {
    setLoading(true);
    try {
      const response = await fetch('/api/admin/discord-webhook', {
        method: 'POST',
  ...
  const testWebhook = async () => {
    await sendNotification('latest-session');
  };
  ...
  useEffect(() => {
    updateCustomPreview();
  }, [customTemplate, selectedSession, sessions]);

  return (
    <div className={`bg-white rounded-lg shadow-md p-6 ${className}`}>
      <h2 className="text-2xl font-bold mb-2 text-gray-800">Announcements</h2>
      <p className="text-sm text-gray-600 mb-6">
        Configure Discord and/or Haven webhooks once. Manual announcements and future automation will
        send to every enabled destination automatically.
      </p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-8">
        <DestinationSettingsSection
          title="Discord"
          description="Send rich embed announcements to a Discord channel webhook."
          webhookLabel="Discord Webhook URL"
          webhookPlaceholder="https://discord.com/api/webhooks/..."
          enableLabel="Enable Discord notifications"
          settings={discordSettings}
          onChange={setDiscordSettings}
        ...
        <DestinationSettingsSection
          title="Haven"
          description="Send markdown announcements to a Haven channel webhook."
          webhookLabel="Haven Webhook URL"
          webhookPlaceholder="https://your-haven-server.com/api/webhooks/..."
          enableLabel="Enable Haven notifications"
          settings={havenSettings}
          onChange={setHavenSettings}
          onSave={saveHavenSettings}
          loading={loading}
        />
      </div>
      ...
      <div className="mb-8 flex flex-wrap gap-3">
        <button
          type="button"
          onClick={testWebhook}
          disabled={loading || !announcementsEnabled}
          className="px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700 disabled:opacity-50"
        >
          {loading ? 'Testing...' : 'Test Announcements'}
        </button>
        <p className="text-sm text-gray-600 self-center">
          Sends the latest session announcement to all enabled destinations.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 bg-gray-100 border border-gray-300 rounded-lg">
            ...
            <button
              ...
              disabled={loading || !announcementsEnabled || !selectedSession}
        ...
        {showCustomizer && (
          <div className="mt-6 p-6 bg-purple-50 border border-purple-200 rounded-lg">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-purple-700 mb-2">Available Data Tags:</label>
                <div className="bg-white p-4 rounded-md border border-purple-200 text-sm">
                  <div className="grid grid-cols-1 gap-2">
                    ...
                    <div><code className="bg-purple-100 px-1 rounded">{'{finalDay}'}</code> - Final game day</div>
              ...
              <div>
                <label className="block text-sm font-medium text-purple-700 mb-2">Custom Message Template:</label>
                <textarea
                ...
                <div className="mt-4">
                  <label className="block text-sm font-medium text-purple-700 mb-2">Preview:</label>
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  <button
                    ...
                    disabled={loading || !announcementsEnabled || !selectedSession || !customTemplate}
            ...
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
              <p className="text-sm text-yellow-800">
                Tip: Discord and Haven both support markdown like **bold**, *italic*, and `code`.
              </p>
            </div>
          </div>
        )}
